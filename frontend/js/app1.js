...
class NacoApp {
  ...
  bindEventListeners() {
    ...
    this.$('#nav-find')?.addEventListener('click', () => {
      this.closeSideMenu();
      window.scrollTo({ top: 0, behavior: 'smooth' });
    });
    this.$('#nav-bookings')?.addEventListener('click', () => {
      this.closeSideMenu();
      this.openMyBookings();
    });
    this.$('#nav-favorites')?.addEventListener('click', () => {
      this.closeSideMenu();
      this.openFavorites();
    });
    this.$('#nav-settings')?.addEventListener('click', () => {
      this.closeSideMenu();
      this.openSettings();
    });
    this.$('#close-side')?.addEventListener('click', () => this.closeSideMenu());
  }
  ...
  renderQuickCategories() {
    ...
    const categories = [
      ...
      { name: 'Plumber', icon: 'fas fa-faucet' },
      { name: 'Carpenter', icon: 'fas fa-hammer' },
      { name: 'Tailor', icon: 'fas fa-tshirt' },
      { name: 'Painter', icon: 'fas fa-paint-roller' },
      { name: 'Welder', icon: 'fas fa-fire' }
    ];

    container.innerHTML = categories.map(cat => `
      <div class="chip" data-cat="${cat.name}">
  ...
  async openFavorites() {
    ...
    if (this.state.favorites.size === 0) {
      this.showToast('You have no saved favorites yet', 'info');
      return;
    }

    try {
      const favorites = await Promise.all(
        [...this.state.favorites].map(id => API.getArtisanById(id))
      );

      const html = `
        <div class="favorites-modal">
          <h2>Your Favorites</h2>
          <p class="muted">Artisans you've saved for quick access</p>
          
          <div class="favorites-list">
            ${favorites.map(artisan => `
              <div class="artisan-card" data-id="${artisan.id}">
                ...
                <div class="artisan-info">
                  ...
                  <div class="artisan-meta">
                    ${this.escapeHtml(artisan.skill)} • ${this.escapeHtml(artisan.location)}
                  </div>
                  <div class="muted">
      ...
      this.showModal(html, () => {
        this.$$('.artisan-card').forEach(card => {
          card.addEventListener('click', () => this.openArtisanProfile(card.dataset.id));
        });
  ...
  openPremiumUpgrade() {
    ...
    <br>
          <div class="price-card">
            <h3>3 Months</h3>
            <div class="price">₦9000<span>/3Months</span></div>
            <div class="savings">Save ₦1,500</div>
    ...
    <div class="price-card">
            <h3>6 Months</h3>
            <div class="price">₦21,000<span>/6Months</span></div>
            <div class="savings">Save ₦3,000</div>
            <button class="btn-primary premium-subscribe" data-plan="yearly">Choose Plan</button>
          </div>
        </div>
    <br>
    <div class="price-card">
            <h3>Yearly</h3>
            <div class="price">₦35,000<span>/year</span></div>
            <div class="savings">Save ₦7,000</div>
            <button class="btn-primary premium-subscribe" data-plan="yearly">Choose Plan</button>
          </div>
        </div>
<br>
      ...
      <div class="settings-modal">
        ...
        <div class="settings-section">
          ...
          <label class="setting-item">
            ...
            <span>Share my location for better matches</span>
          </label>
        </div>
        ...
        <div class="settings-section">
          <h4>Appearance</h4>
          <label class="setting-item">
            <input type="checkbox" id="dark-mode" ${this.state.isDarkMode ? 'checked' : ''}>
        ...
        <div style="text-align: center; margin-top: 20px;">
          ...
          <button id="close-settings" class="link-btn">Cancel</button>
        </div>
      </div>
    `;

    this.showModal(html, () => {
      this.$('#dark-mode')?.addEventListener('change', (e) => {
        this.toggleTheme();
      });
      this.$('#save-settings')?.addEventListener('click', () => {
